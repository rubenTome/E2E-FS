...
tracker = EmissionsTracker(log_level="warning", output_file=path + "/info_" + precision + "/emissions_script_e2efs_ranking_transfer_cifar100_" + precision + ".csv")
tracker.start()
...
def main():
    ...
    print('reps : ', reps)
    name = 'cifar100_' + classifier_network + '_r_' + str(regularization)
    print(name)
    model_kwargs = {
        ...
        for r in range(reps):
            ...
            if os.path.exists(temp_filename):
                ...
                for fs_r in range(fs_reps):
                    ...
                    cont_seed += 1
                    classifier = load_model(fs_filename) if warming_up else getattr(network_models, fs_network)(
                    ...
                    optimizer = custom_optimizers.E2EFS_SGD(e2efs_layer=e2efs_layer)  # optimizers.adam(lr=1e-2)
