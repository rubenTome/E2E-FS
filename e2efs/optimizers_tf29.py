...
class E2EFS_Lossscaleoptimizer(LossScaleOptimizer):
    def __init__(self, inner_optimizer, e2efs_layer, th=.1, kwargs={}, e2efs_lr=0.01, beta_1=0.5, beta_2=0.999,
                 amsgrad=False):
        if (len(kwargs) == 3):
            super(E2EFS_Lossscaleoptimizer, self).__init__(inner_optimizer, dynamic=kwargs["dynamic"],
                initial_scale=kwargs["initial_scale"], dynamic_growth_steps=kwargs["dynamic_growth_steps"])
        else:
    ...
    def set_weights(self, weights):
        ...
        if len(weights) == 3 * num_vars + 1:
            weights = weights[:len(params)]
        super(E2EFS_Lossscaleoptimizer, self).set_weights(weights)
    ...
    def _resource_apply_dense(self, grad, var, apply_state=None):
        if 'e2efs' in var.name:
            ...
            return super(E2EFS_Lossscaleoptimizer, self)._resource_apply_dense(grad, var, apply_state)
