...
tracker = EmissionsTracker(log_level="warning", output_file=path + "/info_" + precision + "/emissions_script_e2efs_ranking_mnist_" + precision + ".csv")
tracker.start()
        ...
        for e2efs_class in e2efs_classes:
            ...
            for r in range(reps):
                ...
                for i, factor in enumerate([.05, .1, .25, .5]):
                    ...
                    optimizer = optimizers.Adam(lr=1e-3)  # optimizers.adam(lr=1e-2)
                    model.compile(loss='categorical_crossentropy', optimizer=optimizer, metrics=['acc'])
